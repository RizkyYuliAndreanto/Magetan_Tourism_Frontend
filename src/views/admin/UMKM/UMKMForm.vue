<template>
  <div class="form-card">
    <div class="form-header">
      <h3 class="form-title">{{ isEditing ? 'Edit UMKM' : 'Tambah UMKM Baru & Galeri' }}</h3>
    ...
    <form @submit.prevent="submitForm" class="form-container">
      <div class="form-section">
        <h4 class="section-title"><i class="fas fa-store"></i> Detail UMKM</h4>
        <div class="form-grid">
          <div class="form-group">
            <label for="nama_umkm">Nama UMKM <span class="required">*</span></label>
            <input type="text" id="nama_umkm" v-model="formData.nama_umkm" class="form-input" required>
          </div>
          <div class="form-group">
            <label for="jenis_usaha">Jenis Usaha <span class="required">*</span></label>
            <input type="text" id="jenis_usaha" v-model="formData.jenis_usaha" class="form-input" required>
          </div>
          <div class="form-group span-2">
            <label for="deskripsi_umkm">Deskripsi UMKM <span class="required">*</span></label>
            <textarea id="deskripsi_umkm" v-model="formData.deskripsi_umkm" class="form-input" rows="5" required></textarea>
          </div>
          <div class="form-group span-2">
            <label for="alamat_umkm">Alamat UMKM <span class="required">*</span></label>
            <textarea id="alamat_umkm" v-model="formData.alamat_umkm" class="form-input" rows="3" required></textarea>
          </div>
          <div class="form-group">
            <label for="kontak_umkm">Kontak UMKM</label>
            <input type="text" id="kontak_umkm" v-model="formData.kontak_umkm" class="form-input">
          </div>
          <div class="form-group">
            <label for="website_umkm">Website UMKM</label>
            <input type="url" id="website_umkm" v-model="formData.website_umkm" class="form-input">
          </div>
          <div class="form-group span-2">
            <label for="gambar_produk_utama">Gambar Produk Utama <span class="required" v-if="!isEditing">*</span></label>
            <div class="file-drop-area" @dragover.prevent @drop="handleDropHeroFile">
              <div class="file-drop-content">
                <i class="fas fa-upload file-icon"></i>
                <span class="file-message" v-if="!formData.preview_gambar_produk_utama">Drag files to upload or <label for="gambar_produk_utama" class="file-link">browse</label></span>
                <input type="file" id="gambar_produk_utama" @change="handleHeroFileUpload" class="file-input" accept="image/*" :required="!isEditing">
              </div>
              <div v-if="formData.preview_gambar_produk_utama" class="image-preview">
                <img :src="formData.preview_gambar_produk_utama" alt="Gambar Produk Utama Preview">
      ...
      <div class="form-actions">
        ...
        <button type="submit" class="action-button save-button">
          <i class="fas fa-save"></i> {{ isEditing ? 'Simpan Perubahan' : 'Simpan UMKM & Galeri' }}
        </button>
      </div>
    </form>
  </div>
...
const submitForm = () => {
  ...
  for (const key in formData.value) {
    if (key !== 'preview_gambar_produk_utama' && formData.value[key] !== null) {
