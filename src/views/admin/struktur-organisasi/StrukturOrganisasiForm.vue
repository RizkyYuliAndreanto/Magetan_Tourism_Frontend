      ...
      <div class="form-section">
        ...
        <div class="form-grid">
          ...
          <div class="form-group span-2">
              ...
              <div class="file-drop-content">
                <template v-if="!previewFile">
                  ...
                  <span class="file-message">
                    ...
                    <label for="gambar_struktur_organisasi" class="file-link">browse</label>
                  </span>
                  <input
                    type="file"
                    id="gambar_struktur_organisasi"
                    @change="handleFileUpload"
                    class="file-input"
                    accept="image/*"
                    :required="!isEditing && !formData.gambar_struktur_path" />
                </template>
                <template v-else>
            ...
            <div v-if="isEditing && formData.gambar_struktur_path && !formData.gambar_struktur_organisasi" class="image-preview-main-existing">
              <h4 class="preview-subtitle">Gambar Saat Ini:</h4>
              <img :src="getFilePath(formData.gambar_struktur_path)" alt="Struktur Organisasi" class="hero-image-preview" />
            </div>
          </div>
        </div>
      </div>
...
watch(() => props.initialData, (newVal) => {
  ...
  if (
    formData.value.gambar_struktur_path &&
    typeof formData.value.gambar_struktur_path === 'string'
  ) {
    previewFile.value = `http://localhost:5000${formData.value.gambar_struktur_path}`;
  } else {
...
const removeFile = () => {
  ...
  formData.value.gambar_struktur_organisasi = null;
  previewFile.value = null;
};
...
const submitForm = () => {
  ...
  for (const key in formData.value) {
    if (key !== 'gambar_struktur_path' && formData.value[key] !== null) {
...
.image-preview-main, .file-preview-main, .image-preview-main-existing {
  width: 100%;
  position: relative;
  border-radius: 8px;
  overflow: hidden;
