<template>
  <div class="detail-page-container">
    <div class="content-wrapper">
      ...
      <div v-if="!loading && !error && strukturAnggotaList.length === 0" class="state-message empty-message">
        <p>📋 Belum ada daftar anggota struktur yang tersedia saat ini.</p>
      </div>

      <div v-if="strukturAnggotaList.length > 0 && !loading" class="anggota-content">
        
        <div v-if="kepalaDinas" class="anggota-section">
          <h3 class="anggota-grid-title">Kepala Dinas</h3>
          <div class="anggota-flex-center">
            <div class="anggota-card kepala-dinas-card" @click="openAnggotaDetailModal(kepalaDinas)">
              <div class="anggota-photo-container kepala-dinas-photo">
                <img 
                  ...
                  class="anggota-photo"
                >
                <div v-else class="anggota-photo-placeholder kepala-dinas-photo-placeholder">
                  <i class="fas fa-user-circle"></i>
                </div>
              </div>
              <h2 class="anggota-name kepala-dinas-name">{{ kepalaDinas.nama_anggota }}</h2>
              <p class="anggota-jabatan kepala-dinas-jabatan">{{ kepalaDinas.jabatan }}</p>
            </div>
          </div>
        </div>
        ...
        <div v-if="sekretaris" class="anggota-section">
          ...
          <div class="anggota-flex-center">
            <div class="anggota-card sekretaris-card" @click="openAnggotaDetailModal(sekretaris)">
              <div class="anggota-photo-container sekretaris-photo">
                <img 
                  ...
                  class="anggota-photo"
                >
                <div v-else class="anggota-photo-placeholder sekretaris-photo-placeholder">
                  <i class="fas fa-user-circle"></i>
                </div>
              </div>
              <h2 class="anggota-name sekretaris-name">{{ sekretaris.nama_anggota }}</h2>
              <p class="anggota-jabatan sekretaris-jabatan">{{ sekretaris.jabatan }}</p>
            </div>
          </div>
        </div>
  ...
  <div v-if="showAnggotaModal" class="anggota-modal-overlay" @click.self="closeAnggotaDetailModal">
    <div class="anggota-modal-content">
      ...
      <div v-if="currentAnggotaDetail" class="modal-body-content">
        ...
        <h2 class="modal-anggota-name">{{ currentAnggotaDetail.nama_anggota }}</h2>
        <p class="modal-anggota-jabatan">{{ currentAnggotaDetail.jabatan }}</p>
        
        <div v-if="currentAnggotaDetail.deskripsi_tugas" class="modal-anggota-deskripsi">
...
const fetchStrukturAnggotaData = async () => {
  ...
  try {
    ...
    if (response.data) {
      strukturAnggotaList.value = response.data.sort((a, b) => a.urutan_tampilan - b.urutan_tampilan);
    } else {
...
const kepalaDinas = computed(() => {
  return strukturAnggotaList.value.find(anggota => anggota.jabatan.toLowerCase().includes('kepala dinas'));
});

const sekretaris = computed(() => {
  return strukturAnggotaList.value.find(anggota => anggota.jabatan.toLowerCase().includes('sekretaris'));
});
...
.state-message { text-align: center; font-size: 1.2rem; color: #6c757d; margin: 50px 0; }
.state-message.error-message { color: #dc3545; }
.state-message.empty-message { color: #ffc107; }
.spinner { border: 4px solid rgba(0, 0, 0, 0.1); width: 36px; height: 36px; border-radius: 50%; border-left-color: #0077b6; animation: spin 1s ease infinite; margin: 0 auto 15px; }
...
@media (max-width: 768px) {
  ...
  .kepala-dinas-card, .sekretaris-card {
    max-width: 100%;
  }
  .anggota-modal-content {
    padding: 30px 20px;
